...
public class EnemyScrips : MonoBehaviour
{
    private float range;
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        {
            Vector3 contactPoin = collision.contacts[0].point;
            Vector3 center = collision.collider.bounds.center;

            targetCollision = true;

            bool right = contactPoin.x > center.x;
            bool left = contactPoin.x < center.x;
            bool top = contactPoin.y > center.y;
            bool bottom = contactPoin.y < center.y;
    ...
    public void TakeDamage(int amount)
    {
        ...
        if (health < 0)
        {
            ...
            target.GetComponent<PlayerScrips>().GainExperience(100);
